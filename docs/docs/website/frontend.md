...
This document delves deeper into the frontend of the website.

## Frameworks

The major framework used in the frontend is [React](https://react.dev/), the de-facto
...
React does not handle the styling (aka the CSS), so we use [TailwindCSS](https://tailwindcss.com) in order
...
color to a shade of red. The UI components such as buttons, are not provided by TailwindCSS, but instead come
from [NextUI](https://nextui.org/)

:::tip

It would be extremely helpful to learn how to use TailwindCSS, as there is simply not enough time to cover how to teach it within the project itself.

It's not required to know how to use TailwindCSS, but it is highly recommended to understand how does it work before getting started.
...
The design of the website was done using [Figma](https://figma.com/). Members are not required
